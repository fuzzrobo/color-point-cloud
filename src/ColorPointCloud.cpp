        ...
        // Create camera object, create subscriber to image and camera_info
        {
            for (const auto &camera_topic : camera_topics_)
            {
                ...
                this->camera_info_subscribers_.push_back(this->create_subscription<sensor_msgs::msg::CameraInfo>(
                    camera_info_topic, rclcpp::SensorDataQoS(), [this, camera_info_topic, camera_topic](const sensor_msgs::msg::CameraInfo::ConstSharedPtr &msg)
                    {
    ...
    void ColorPointCloud::point_cloud_callback(const sensor_msgs::msg::PointCloud2::ConstSharedPtr &msg)
    {
        ...
        std::for_each(camera_type_stdmap_.begin(), camera_type_stdmap_.end(),
                      [this, &cloud_color_msg, msg](std::pair<std::string, CameraTypePtr> pair)
                      {
                          ...
                          if (!pair.second->set_cv_image(pair.second->get_image_msg(), image_type_))
                              return;
                          ...
                          for (size_t i = 0; i < cloud.getPointCount(); ++i)
                          {
                              ...
                              // u,v (double) を計算した後に
                              int iu = (int)std::lround(x);
                              int iv = (int)std::lround(y);

                              const cv::Mat &img = pair.second->get_cv_image();
                              if (img.empty())
                              {
                                  iter_r[0] = 0.0f;
                                  iter_g[0] = 0.0f;
                              ...
                              const int W = img.cols;//(int)pair.second->get_image_width();
                              const int H = img.rows;//(int)pair.second->get_image_height();

                              if (iu < 0 || iu >= W || iv < 0 || iv >= H ||
                                  point3d_transformed_camera[2] < 1e-6)
                              {
                              ...
                              {
                                  const cv::Vec3b color = img.at<cv::Vec3b>(iv, iu);
